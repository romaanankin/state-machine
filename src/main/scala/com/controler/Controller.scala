...
class Controller(implicit entityService: EntityService,
                 stateMatrixService: StateMatrixService, historyService: HistoryService) {
  ...
  val route: Route =
    concat(
      ...
      get {
        path("history") {
          val history: Future[Option[List[String]]] = Future {historyService.fetchAll()}
